...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        if not await _binary_available("tmux", remote, req.ssh_port):
            return {
                "ok": False,
    ...
    async def _serve_crash_watchdog(
        ...
        _waits = [25, 35, 60, 180]
        # Build the capture-pane command once and reuse on each tick.
        if remote:
            ssh_args = ["ssh"]
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if _needs_binary(req.cmd, "docker") and not await _binary_available("docker", remote, req.ssh_port):
            return {
                "ok": False,
